...
pub fn command() -> App<'static, 'static> {
    SubCommand::with_name("pull").help("Pull the newest revision from the remote.")
...
pub fn go(repository: &mut Repository, matches: &ArgMatches) -> Result<()> {
    let remote_name = match repository.refs.head {
        Head::Detached(hash) => bail!("Head is detached!"),
        Head::LocalRef(ref branch) => bail!("Head is local!"),
        Head::RemoteRef(ref remote, ref branch) => remote.clone(),
    };
