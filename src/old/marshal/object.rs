...
use std::{iter, slice, borrow::Cow, collections::btree_map::{self, BTreeMap}, ffi::OsString};
...
impl<'a> RawSmallObject<'a> {
    unsafe fn into_object(self, slice: ArcSlice) -> SmallObject {
        ...
        let offset = slice
            .as_ptr()
            .offset_to(self.chunk.as_ptr())
            .expect("u8 is not a ZST");
...
pub enum RawDataObject<'a> {
    ...
    Small(#[serde(borrow)] RawSmallObject<'a>),
...
pub enum RawObject<'a> {
    ...
    Data(#[serde(borrow)] RawDataObject<'a>),
...
//     pub fn iter_deps(&self) -> ObjectDeps {
//         match *self {
//             Object::Data(DataObject::Small(_)) => ObjectDeps::Small,
//             Object::Data(DataObject::Large(ref large_object)) => {
//                 ObjectDeps::Large(large_object.children.iter())
//             }
//             Object::Subtree(ref subtree_object) => {
//                 ObjectDeps::Subtree(subtree_object.entries.iter())
//             }
//             Object::Commit(ref commit_object) => ObjectDeps::Commit(
//                 iter::once(&commit_object.subtree).chain(commit_object.parents.iter()),
//             ),
//         }
//     }
}
...
// #[derive(Debug, Clone)]
// pub enum ObjectDeps<'a> {
//     Small,
//     Large(slice::Iter<'a, (u64, ObjectHash)>),
//     Subtree(btree_map::Iter<'a, OsString, SubtreeEntry>),
//     Commit(iter::Chain<iter::Once<&'a ObjectHash>, slice::Iter<'a, ObjectHash>>),
// }
