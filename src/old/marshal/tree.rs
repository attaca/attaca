...
use errors::Error;
use marshal::{Marshaller, ObjectHash, SubtreeEntry, SubtreeObject};
use trace::Trace;
...
impl Tree {
    ...
    ) -> Result<SubtreeEntry, Error> {
        let node = this.lock().unwrap()[node_id].take().unwrap();
        match node {
            Node::Opaque(subtree_entry) => await!(
                marshaller
                    .process(subtree_entry.hash())
                    .map(|_| subtree_entry)
            ),
            Node::Transparent(entries) => {
                ...
                let future_node_hash = stream::futures_unordered(future_entries)
