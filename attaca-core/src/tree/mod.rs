...
use hasher::Hasher;
use object::{SubtreeEntry, SubtreeObject};
            ...
            match node {
                ...
                Node::Transparent(entries) => {
                    self.stack.extend(
                        entries
                            .into_iter()
                            .map(|(component, node_id)| (path.join(component), node_id)),
                    );
...
impl Tree {
    fn append(&mut self, subtree: Tree) -> NodeId {
        ...
        self.arena
            .entries
            .extend(subtree.arena.entries.into_iter().map(|mut entry| {
                match entry {
                    Some(Node::Transparent(ref mut entries)) => {
        ...
        I::Item: Borrow<OsStr> + Into<OsString>,
    {
        ...
        let preliminary = {
            ...
            loop {
                match iter.next() {
                    Some(immediate_component) => match self.arena[current] {
                        Some(Node::Opaque(_)) => break Preliminary::Blocked(immediate_component),
        ...
        match preliminary {
            Preliminary::Blocked(immediate_component) => Err(Blocked {
                blocking_id: current,
                immediate_component,
                remaining_path: path,
                tree: self,
            }),
            Preliminary::Occupied => Ok(Entry::Occupied(OccupiedEntry {
                node_id: current,
                empty_iter: path,
                tree: self,
            })),
    ...
    fn marshal_inner(
        ...
        Box::new(async_block! {
            ...
            match node {
                Node::Opaque(subtree_entry) =>
                    await!(marshaller.process(subtree_entry.hash()).map(|_| subtree_entry)),
                Node::Transparent(entries) => {
    ...
    #[async]
    pub fn marshal(self, marshaller: Hasher) -> Result<ObjectHash, Error> {
    ...
    quickcheck! {
        ...
        fn from_and_into_iter(objects: Vec<Vec<String>>) -> TestResult {
            let mut paths = objects
                .into_iter()
                .map(|p| p.into_iter().collect::<PathBuf>())
                .collect::<Vec<_>>();
            ...
            let tree = paths
                .iter()
                .cloned()
                .map(|path| (path, SubtreeEntry::File(ObjectHash::zero(), 0)))
                .collect::<Tree>();
            ...
            if pre_hashset == post_hashset {
                ...
                TestResult::error(
                    format!(
                        "Mismatch: {:?}",
                        pre_hashset
                            .symmetric_difference(&post_hashset)
                            .collect::<Vec<_>>(),
                    )
                )
            }
