...
pub struct GenericSplitter<T, F, H, I, S>
where
    F: Fn(T) -> (H, I),
    H: Deref<Target = [u8]>,
    S: Iterator<Item = T>,
...
impl<T, F, H, I, S> GenericSplitter<T, F, H, I, S>
where
    F: Fn(T) -> (H, I),
    H: Deref<Target = [u8]>,
    S: Iterator<Item = T>,
...
impl<T, F, H, I, S> Iterator for GenericSplitter<T, F, H, I, S>
where
    F: Fn(T) -> (H, I),
    H: Deref<Target = [u8]>,
    S: Iterator<Item = T>,
            ...
            match self.iterator.as_mut().map(Iterator::next) {
                Some(Some(input)) => {
                    ...
                    for (i, &b) in (buf_len..).zip(&self.buf[buf_len..]) {
                        ...
                        if i >= self.window {
                            self.acc = self.acc.wrapping_sub(self.buf[i - self.window] as u64);
                        }
                    }

                    self.acc &= (1 << self.modulus_power) - 1;

                    if self.buf.len() >= self.window {
                        let new_buf_len = self.buf.len();
                        self.buf.drain(..new_buf_len - self.window);
                    }

                    if self.group.len() >= self.minimum && self.acc == self.constant {
